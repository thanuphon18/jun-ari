...
async function seed() {
  ...
  const inventoryRows = insertedVariants.map(function(v, i) {
    return {
      variant_id: v.id,
      warehouse: "main",
      qty_on_hand: 50 + (i * 15),
  ...
  for (var i = 0; i < 12; i++) {
    var isB2B = i >= 8;
    var userId = isB2B ? distributorId : customerId;
    var si = i % statuses.length;
    var date = new Date();
  ...
  for (const order of insertedOrders) {
    var v1 = insertedVariants[Math.floor(Math.random() * insertedVariants.length)];
    var v2 = insertedVariants[Math.floor(Math.random() * insertedVariants.length)];
    var prod1 = insertedProducts.find(function(p) { return p.id === v1.product_id; });
    var prod2 = insertedProducts.find(function(p) { return p.id === v2.product_id; });
    itemRows.push(
      {
        order_id: order.id,
        variant_id: v1.id,
        product_name: prod1 ? prod1.slug.replace(/-/g, " ") : "Product",
        variant_name: v1.sku.endsWith("-SM") ? "30 Count" : "90 Count",
        sku: v1.sku,
        quantity: 1 + Math.floor(Math.random() * 3),
        unit_price: prod1 ? prod1.base_price : 29.99,
        total_price: prod1 ? prod1.base_price : 29.99,
      },
      {
        order_id: order.id,
        variant_id: v2.id,
        product_name: prod2 ? prod2.slug.replace(/-/g, " ") : "Product",
        variant_name: v2.sku.endsWith("-SM") ? "30 Count" : "90 Count",
        sku: v2.sku,
        quantity: 1 + Math.floor(Math.random() * 2),
        unit_price: prod2 ? prod2.base_price : 24.99,
        total_price: prod2 ? prod2.base_price : 24.99,
      }
    );
  }
